		...
		public function getNextChunk() {
			...
			if ($this->mediapipes != null) {
				...
				if ((ord(substr($metadata, 0, 1)) * 16) != (strlen($metadata) - 1)) {
					Logger::debug("Metadata did not pass the parse check.  It is not going to be sent in this chunk.  The size byte times 16 does not match ".(strlen($metadata) - 1).".  Raw data:  [{".ord(substr($metadata, 0, 1))."}]".substr($metadata, 1));
					$metadata = chr(0);
				}
				$data = null;
